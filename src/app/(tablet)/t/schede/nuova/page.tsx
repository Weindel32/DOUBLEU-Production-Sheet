...
function NuovaSchedaForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/t/schede/${scheda.id}`);
    } catch {
  ...
  return (
    <div className="p-5 max-w-2xl mx-auto">
      <Link href="/t/schede" className="flex items-center gap-2 text-sm text-[#8ba3c7] hover:text-white mb-6 transition-colors">
        <ArrowLeft size={16} />
        Torna alle schede
      </Link>
...
export default function TabletNuovaSchedaPage() {
  return (
    <Suspense>
      <NuovaSchedaForm />
    </Suspense>

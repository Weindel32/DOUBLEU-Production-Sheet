...
export default function NuovoLogoPage() {
  ...
  return (
    <div className="p-6 max-w-xl">
      <Link href="/loghi" className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-800 mb-6">
        <ArrowLeft size={16} /> Torna alla libreria
      </Link>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Aggiungi logo</h1>
      <form onSubmit={handleSubmit} className="card space-y-4">
        <div>
          <label className="text-sm text-gray-600 block mb-1">Nome logo *</label>
        ...
        <div>
          <label className="text-sm text-gray-600 block mb-1">Tipo applicazione *</label>
        ...
        <div>
          <label className="text-sm text-gray-600 block mb-1">File immagine *</label>
          ...
          {fileUrl ? (
            <div className="border border-green-200 bg-green-50 rounded-lg p-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <button type="button" onClick={() => setFileUrl("")} className="text-xs text-gray-400 hover:text-red-500 transition-colors">Rimuovi</button>
              </div>
            </div>
          ) : (
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
              className="w-full border-2 border-dashed border-gray-200 rounded-lg py-6 flex flex-col items-center gap-2 text-sm text-gray-400 hover:border-blue-300 hover:text-blue-500 transition-colors disabled:opacity-50">
        ...
        <div className="flex gap-3 pt-2">
          <Link href="/loghi" className="flex-1 text-center border border-gray-300 text-gray-600 px-4 py-2 rounded-lg text-sm hover:bg-gray-50 transition-colors">Annulla</Link>

...
export default function NuovoClientePage() {
  ...
  return (
    <div className="p-6 max-w-xl">
      <Link href="/clienti" className="flex items-center gap-2 text-sm text-[#8ba3c7] hover:text-white mb-6">
        <ArrowLeft size={16} /> Torna ai clienti
      </Link>
      <h1 className="text-2xl font-bold text-white mb-6">Nuovo cliente</h1>
      <form onSubmit={handleSubmit} className="card space-y-4">
        <div>
          <label className="text-sm text-[#8ba3c7] block mb-1">Nome / Club *</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-sm text-[#8ba3c7] block mb-1">Email</label>
            <input type="email" value={form.email} onChange={(e) => set("email", e.target.value)}
              placeholder="es. info@club.it"
              className="w-full border border-white/15 rounded-lg px-3 py-2 text-sm focus:border-blue-400 outline-none" />
          </div>
          <div>
            <label className="text-sm text-[#8ba3c7] block mb-1">Telefono</label>
        ...
        <div>
          <label className="text-sm text-[#8ba3c7] block mb-1">Indirizzo</label>
        ...
        <div className="flex gap-3 pt-2">
          <Link href="/clienti" className="flex-1 text-center border border-white/15 text-[#8ba3c7] px-4 py-2 rounded-lg text-sm hover:bg-white/[0.03] transition-colors">Annulla</Link>
